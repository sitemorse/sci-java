...
public class SCIClient {
	...
	public String performTest(String url, String[] hostNames, String view)
		...
		if (!(view.equals("report") || view.equals("snapshot-page") || view
				.equals("snapshot-source")))
			throw new SCIException("Invalid 'view' parameter");
		try {
			if (sciProxyHostname != null) {
				sock = new Socket();
				sock.connect(new InetSocketAddress(getSciProxyHostname(),
						getSciProxyPort()), SCI_CONNECT_TIMEOUT);
				...
				sciOut.write("CONNECT " + getServerHostname() + ":"
						+ getServerPort() + " HTTP/1.0" + CRLF + CRLF);
				sciOut.flush();
				line = sciIn.readLine();
				if (line == null)
				...
				if (serverSecure)
					...
					sock = new Socket();
				sock.connect(new InetSocketAddress(getServerHostname(),
						getServerPort()), SCI_CONNECT_TIMEOUT);
			}
	...
	private String GenerateAuthResponse(String challenge) throws SCIException {
		try {
			SecretKeySpec signingKey = new SecretKeySpec(licenceKey
					.substring(8).getBytes(SCI_CHARSET), SCI_HASH_ALGORITHM);
			Mac mac = Mac.getInstance(SCI_HASH_ALGORITHM);
			mac.init(signingKey);
			return licenceKey.substring(0, 8)
					+ Hexify(mac.doFinal(challenge.getBytes(SCI_CHARSET)));
			...
			while (true) {
				...
				if (!url.getProtocol().equals("http")
					...
					sciOut.write("XSCI badscheme " + "URL scheme '"
							+ url.getProtocol() + "' not allowed" + CRLF);
					sciOut.flush();
					continue;
				}
					...
					sock.connect(
							new InetSocketAddress(url.getHost(),
									url.getPort() == -1 ? url.getDefaultPort()
											: url.getPort()), WEB_TIMEOUT);
					webIn = new BufferedReader(new InputStreamReader(
							sock.getInputStream(), SCI_CHARSET));
					webOut = new BufferedWriter(new OutputStreamWriter(
							sock.getOutputStream(), SCI_CHARSET));
